...
export default function SearchInput({ ingredients, selectedIds, onSelect, translate, placeholder, disabled }: Props) {
  ...
  // Before the first ingredient is picked, show top-frequency defaults with no typing required
  ...
  function commit(ingredient: Ingredient) {
    ...
    setQuery("");
    setActiveIdx(-1);
    // Close dropdown only after first pick; stay open for subsequent additions
    setOpen(false);
