...
face detection (no humans in frame) → rembg background removal → QC gate →
uniform tile). Tile criteria: subject centered filling ~80% of the frame,
ideally a single specimen, no people, and the **food form rather than the live
animal** (chicken → roast, not hen — enforced via overrides). Run via the manual
