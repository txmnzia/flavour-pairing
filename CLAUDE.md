...
- `pipeline/curation.json` — manual curation decisions (deleted, merged). Applied at deploy time. Currently produces **1,081 ingredients** in the live app.
- `pipeline/merges.json` — one-time normalisation merges (6,649 → 3,517). Read-only.
- Deployed ingredient list (1,081) is derived at build time; no single file lists it.
